...
<template>
  <div class="l2-header">
    ...
    <div class="header-actions">
      <!-- 1. 修改點：為所有按鈕添加 .button-reset, .interactive-item, .rounded-sm -->
      <button 
        v-if="mainStore.previousSidebarMode" 
        @click="restorePreviousMode" 
        :title="`返回${previousModeText}模式`"
        class="button-reset interactive-item rounded-sm"
      >
      ...
      <button @click="onCreateFile" title="新增檔案" class="button-reset interactive-item rounded-sm">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
          <path d="M8 6.5a.5.5 0 0 1 .5.5v1.5H10a.5.5 0 0 1 0 1H8.5V11a.5.5 0 0 1-1 0V9.5H6a.5.5 0 0 1 0-1h1.5V7a.5.5 0 0 1 .5-.5z"/>
          <path d="M14 4.5V14a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h5.5L14 4.5zm-3 0A1.5 1.5 0 0 1 9.5 3V1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V4.5h-2z"/>
        </svg>
      </button>
      <button @click="onCreateFolder" title="新增資料夾" class="button-reset interactive-item rounded-sm">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
      ...
      <button @click="onLoadFiles" :disabled="isLoading" title="重新載入資料夾" class="button-reset interactive-item rounded-sm">
         <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
          <path fill-rule="evenodd" d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2v1z"/>
          <path d="M8 4.466V.534a.25.25 0 0 1 .41-.192l2.36 1.966c.12.1.12.284 0 .384L8.41 4.658A.25.25 0 0 1 8 4.466z"/>
        </svg>
      </button>
      <button @click="onToggleCollapse" title="收合側欄" class="collapse-button button-reset interactive-item rounded-sm">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
...
/* 2. 簡化點：大部分樣式已由通用 class 取代 */
.header-actions button {
  /* display, align, justify, background, border, cursor, border-radius, transition 已被取代 */
  color: var(--text-secondary);
  padding: 5px;
  margin-right: 4px;
}

/* hover 效果由 .interactive-item 提供 */
